...
public class LightCollector extends BlockWithEntity implements BlockEntityProvider {
    public static final MapCodec<LightCollector> CODEC = LightCollector.createCodec(LightCollector::new);

    public LightCollector(Settings settings) {
        super(settings);
    }
    ...
    public @Nullable BlockEntity createBlockEntity(BlockPos pos, BlockState state) {
        return new LightCollectorBlockEntity(pos, state);
    }
    ...
    protected void onStateReplaced(BlockState state, World world, BlockPos pos, BlockState newState, boolean moved) {
        if (state.getBlock() != newState.getBlock()) {
            ...
            if (blockEntity instanceof LightCollectorBlockEntity) {
                ItemScatterer.spawn(world, pos, ((LightCollectorBlockEntity) blockEntity));
    ...
    protected ItemActionResult onUseWithItem(ItemStack stack, BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit) {
        if (world.getBlockEntity(pos) instanceof LightCollectorBlockEntity pedestal) {
