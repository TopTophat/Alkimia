...
public class CondenserInputBlockEntity extends BlockEntity implements TickableBlockEntity {
    ...
    public CondenserRecipe lastRecipe = null;

    public CondenserInputBlockEntity(BlockPos pos, BlockState state) {
        super(ModBlockEntities.CONDENSER_INPUT_BE, pos, state);
    }
    ...
    public CondenserRecipe findMatchingRecipe()
    {
        System.out.println("Finding recipe");
        boolean isGood;
        assert world != null;
        if (world.getBlockEntity(pos.up()) instanceof CondenserOutputBlockEntity out)
       {
           for (int i = 0; i < CondenserInput.condenser_recipe_book.size(); i++) {
               isGood = true;
               CondenserRecipe recipe = CondenserInput.condenser_recipe_book.get(i);
               ...
               if (isGood)
               {
                   if (isGood)
                   {
                       ...
                       {
                           System.out.println("All fluids are in the recipe");
                       }
                       else
                       {
                           isGood = false;
    ...
    public boolean isThisFluidInTheRecipe(FluidVariant fluid, CondenserRecipe recipe)
    {
    ...
    public int recipeOutputSum(CondenserRecipe recipe)
    {
        int j = 0;
    ...
    private void finishRecipe(CondenserRecipe recipe) {
    ...
    public void tick() {
        if (world != null && !world.isClient)
        {
            ...
            if (world.getBlockState(pos.up()).getBlock() == ModBlocks.CONDENSER_OUTPUT)
            {
                System.out.println("Condensing flask found");
                if (world.getBlockState(pos.down(1)).getBlock() == ModBlocks.STOVE && world.getBlockState(pos.down(1)).get(Stove.IS_LIT) && world.getBlockState(pos.up().up()).getBlock() == ModBlocks.COOLER)
                {
                    System.out.println("Cooler and a lit stove found");
