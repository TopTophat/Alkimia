...
import net.falling_pan.alkimia.block.custom.*;
import net.falling_pan.alkimia.block.entity.ImplementedInventory;
...
public class CentrifugeInputBlockEntity extends BlockEntity implements ImplementedInventory, TickableBlockEntity {
    ...
    public CentrifugeRecipe lastRecipe = null;

    public CentrifugeInputBlockEntity(BlockPos pos, BlockState state) {
        super(ModBlockEntities.CENTRIFUGE_INPUT_BE, pos, state);
    }
    ...
    public CentrifugeRecipe findMatchingRecipe()
    {
        boolean isGood = true;
        assert world != null;
        if (isAssembled() && world.getBlockEntity(pos.east()) instanceof CentrifugeOutBlockEntity out1 && world.getBlockEntity(pos.west()) instanceof CentrifugeOutBlockEntity out2 && world.getBlockEntity(pos.north()) instanceof CentrifugeOutBlockEntity out3 && world.getBlockEntity(pos.south()) instanceof CentrifugeOutBlockEntity out4)
       {
           for (int i = 0; i < CentrifugeInput.centrifuge_recipe_book.size(); i++) {
               isGood = true;
               CentrifugeRecipe recipe = CentrifugeInput.centrifuge_recipe_book.get(i);
                   ...
                   if (isGood)
                   {
                       ...
                       if (isGood)
                       {
                           ...
                           for (int j = 0; j < inventory.size(); j++) {
                                ...
                                {
                                    isGood = false;
                                    System.out.println("not not not good");
                                    break;
                                }
                           }
                           ...
                           if (isGood)
                           {
                               if (out1.amount + recipe.fluidOutputAmounts.get(0) > out1.capacity && out2.amount + recipe.fluidOutputAmounts.get(1) > out2.capacity && out3.amount + recipe.fluidOutputAmounts.get(2) > out3.capacity && out4.amount + recipe.fluidOutputAmounts.get(3) > out4.capacity)
                               {
                                   isGood = false;
                                   System.out.println("not good");
                               }
                               if (isGood && out1.isEmpty() && out2.isEmpty() && out3.isEmpty() && out4.isEmpty())
                               {
    ...
    public boolean isThisFluidInTheRecipe(FluidVariant fluid, CentrifugeRecipe recipe)
    {
    ...
    public boolean isThisItemInTheRecipe(Item item, CentrifugeRecipe recipe, List<Integer> exclude)
    {
        boolean isDetected = false;
        ...
        for (int i = 0; i < recipe.inputItems.size(); i++) {
            ...
            System.out.println("We look for " + recipe.inputItems.get(i));
    ...
    public int whereIsThisItemInTheRecipe(Item item, CentrifugeRecipe recipe, List<Integer> exclude)
    {
        if (item == Items.AIR)
        {
            return 99;
    ...
    public int whereIsThisFluid(FluidVariant fluid, List<FluidVariant> fluids, List<Integer> amounts, List<Integer> capacities, int add)
    {
        for (int i = 0; i < fluids.size(); i++) {
            if (fluid == fluids.get(i))
            {
                if (amounts.get(i) + add <= capacities.get(i))
                {
                    return i;
                }
            }
    ...
    private void finishRecipe(CentrifugeRecipe recipe) {
        if (world != null && !world.isClient)
        {
            ...
            {
                clear();
                out1.setStack(0, recipe.outputItems.get(0).getDefaultStack().copyWithCount(1));
                out2.setStack(0, recipe.outputItems.get(1).getDefaultStack().copyWithCount(1));
                out3.setStack(0, recipe.outputItems.get(2).getDefaultStack().copyWithCount(1));
                out4.setStack(0, recipe.outputItems.get(3).getDefaultStack().copyWithCount(1));
                for (int i = 0; i < recipe.fluidInputAmounts.size(); i++) {
                    switch (whereIsThisFluid(recipe.inputFluids.get(i), List.of(storedFluid1, storedFluid2, storedFluid3, storedFluid4), List.of(amount1, amount2, amount3, amount4), List.of(1000, 1000, 1000, 1000), recipe.fluidInputAmounts.get(i)))
                    {
                ...
                for (int i = 0; i < recipe.fluidOutputAmounts.size(); i++) {
                    switch (whereIsThisFluid(recipe.outputFluids.get(i), List.of(out1.storedFluid, out2.storedFluid, out3.storedFluid, out4.storedFluid), List.of(out1.amount, out2.amount, out3.amount, out4.amount), List.of(1000, 1000, 1000, 1000), recipe.fluidOutputAmounts.get(i)))
                    {
                        case 0 -> out1.storedFluid = recipe.outputFluids.get(i);
                        case 1 -> out2.storedFluid = recipe.outputFluids.get(i);
                        case 2 -> out3.storedFluid = recipe.outputFluids.get(i);
                        case 3 -> out4.storedFluid = recipe.outputFluids.get(i);
                    }
                    switch (whereIsThisFluid(recipe.outputFluids.get(i), List.of(out1.storedFluid, out2.storedFluid, out3.storedFluid, out4.storedFluid), List.of(out1.amount, out2.amount, out3.amount, out4.amount), List.of(1000, 1000, 1000, 1000), recipe.fluidOutputAmounts.get(i)))
                    {
                        case 0 -> out1.amount += recipe.fluidOutputAmounts.get(i);
                        case 1 -> out2.amount += recipe.fluidOutputAmounts.get(i);
                        case 2 -> out3.amount += recipe.fluidOutputAmounts.get(i);
                        case 3 -> out4.amount += recipe.fluidOutputAmounts.get(i);
                    }
                }
                ...
                world.updateListeners(pos.east(), world.getBlockState(pos.east()), world.getBlockState(pos.east()), Block.NOTIFY_ALL);
                world.updateListeners(pos.west(), world.getBlockState(pos.west()), world.getBlockState(pos.west()), Block.NOTIFY_ALL);
                world.updateListeners(pos.north(), world.getBlockState(pos.north()), world.getBlockState(pos.north()), Block.NOTIFY_ALL);
                world.updateListeners(pos.south(), world.getBlockState(pos.south()), world.getBlockState(pos.south()), Block.NOTIFY_ALL);
            }
        }
    }
    ...
    {
        assert world != null;
        if (world.getBlockState(pos.east()).getBlock() == ModBlocks.CENTRIFUGE_OUT && world.getBlockState(pos.west()).getBlock() == ModBlocks.CENTRIFUGE_OUT && world.getBlockState(pos.north()).getBlock() == ModBlocks.CENTRIFUGE_OUT && world.getBlockState(pos.south()).getBlock() == ModBlocks.CENTRIFUGE_OUT)
        {
            if (world.getBlockState(pos.east()).get(Properties.HORIZONTAL_FACING) == Direction.WEST && world.getBlockState(pos.west()).get(Properties.HORIZONTAL_FACING) == Direction.EAST && world.getBlockState(pos.north()).get(Properties.HORIZONTAL_FACING) == Direction.SOUTH && world.getBlockState(pos.south()).get(Properties.HORIZONTAL_FACING) == Direction.NORTH)
            {
                return true;
            }
        }
